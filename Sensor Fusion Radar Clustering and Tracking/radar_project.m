...
for rw1 = train_cells + guard_cells + 1 : (Nr/2) - (train_cells + guard_cells)
  for cl1 = train_band + guard_band + 1 : (Nd) - (train_band + guard_band)
    %Create a vector to store noise_level for each iteration on training cells
    noise_level = zeros(1, 1);

    for rw2 = rw1 - (train_cells + guard_cells) : rw1 + (train_cells + guard_cells)
      for cl2 = cl1 - (train_band + guard_band) : cl1 + (train_band + guard_band)
        if (abs(rw1 - rw2) > guard_cells || abs(cl1 - cl2) > guard_band)
          noise_level = noise_level + db2pow(range_doppler_map(rw2, cl2));
        end
      end
    end
    ...
    cell_under_test = range_doppler_map(rw1,cl1);

    if (cell_under_test < threshold)
      range_doppler_map(rw1, cl1) = 0;
    else
      range_doppler_map(rw1, cl1) = 1;
    end
